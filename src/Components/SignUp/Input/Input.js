...
function Input() {
  ...
  const createUser = (e) => {
    ...
    if (name.length < 2 || name.length > 50) {
      return alert(
        "name should be at least 2 chars long and maximum 50 characters long"
      );
    }
    ...
    if (password === "" || password.length < 6)
      return alert("The password must be of atleast 6 length");
    if (password !== confirPassword) return alert("Password did not match");

    auth
      .createUserWithEmailAndPassword(email, password)
      .then((userCredential) => {
        if (userCredential) history.push("/menteepage");
